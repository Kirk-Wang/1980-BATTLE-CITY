...
export class BotGroupConfig extends Record({
    ...
    public static unwind(botGroupConfig: BotGroupConfig) {
...
export default class StageConfig extends StageConfigRecord {
    ...
    public static parseBrickBits(str: string) {
        if (str.length === 1) {
     ...
     * 老鹰 eagle  E
     */
    public static parseStageMap(map: RawStageConfig["map"]) {
        ...
        return new MapRecord({
            ...
            bricks: Repeat(false, N_MAP.BRICK ** 2)
                .map((_set, index) => bricks.has(index))
                .toList(),
            steels: Repeat(false, N_MAP.STEEL ** 2)
                .map((_set, index) => steels.has(index))
                .toList(),
            rivers: Repeat(false, N_MAP.RIVER ** 2)
                .map((_set, index) => rivers.has(index))
                .toList(),
            snows: Repeat(false, N_MAP.SNOW ** 2)
                .map((_set, index) => snows.has(index))
                .toList(),
            forests: Repeat(false, N_MAP.FOREST ** 2)
                .map((_set, index) => forests.has(index))
                .toList(),
        });
    }

    public static parseStageBots(bots: RawStageConfig["bots"]) {
...
export namespace StageConfigConverter {
    ...
    export function s2e(stage: StageConfig): EditorStageConfig {
        ...
        rivers.forEach((set, t) => {
            if (set) {
                items[t] = new MapItem({ type: "R" });
            }
        });
        forests.forEach((set, t) => {
            if (set) {
                items[t] = new MapItem({ type: "F" });
            }
        });
        snows.forEach((set, t) => {
            if (set) {
                items[t] = new MapItem({ type: "S" });
            }
        });
