...
interface Chars {
    [char: string]: (props: { fill: string }) => JSX.Element;
}
...
const chars: Chars = {
    ...
    1: ({ fill }) => <path className="character-1" fill={fill} d="M4,0 h2 v6 h2 v1 h-6 v-1 h2 v-4 h-1 v-1 h1 v-1" />,
    2: ({ fill }) => (
        <path
    ...
    y: ({ fill }) => (
        <path className="character-y" fill={fill} d="M2,0 h2 v3 h2 v-3 h2 v3 h-1 v1 h-1 v3 h-2 v-3 h-1 v-1 h-1 v-3" />
    ),
    z: ({ fill }) => (
        <path
    ...
    "-": ({ fill }) => <rect className="character-dash" fill={fill} x="1" y="3" width="6" height="2" />,
    "+": ({ fill }) => (
    ...
    ".": ({ fill }) => <rect className="character-dot" x="2" y="5" width="2" height="2" fill={fill} />,
    "?": ({ fill }) => (
    ...
    ["\u2160".toLowerCase()]: ({ fill }) => (
        <path className="character-roman-numeral-one" fill={fill} d="M2,0 h4 v1 h-1 v5 h1 v1 h-4 v-1 h1 v-5 h-1 v-1" />
    ),
    ["\u2161".toLowerCase()]: ({ fill }) => (
...
export default class Text extends React.PureComponent<Props> {
    public static support(char: string) {
