...
class Editor extends React.Component<EditorProps> {
    ...
    public componentDidMount() {
        ...
        this.setState({ name, difficulty, itemList, bots });
    }

    public getT(event: React.MouseEvent<SVGSVGElement>) {
        let totalTop = 0;
        let totalLeft = 0;
    ...
    public setAsCurrentItem(t: number) {
    ...
    public onMouseMove = (event: React.MouseEvent<SVGSVGElement>) => {
        const {
            view,
            popupHandle: { popup },
        } = this.props;
    ...
    public onIncDifficulty = () => {
        const { difficulty } = this.state;
        this.setState({ difficulty: difficulty + 1 });
    };

    public onDecDifficulty = () => {
        const { difficulty } = this.state;
        this.setState({ difficulty: difficulty - 1 });
    };

    public onIncBotLevel = (index: number) => {
    ...
    public onDecBotLevel = (index: number) => {
        const { bots } = this.state;
        this.setState({
    ...
    public onIncBotCount = (index: number) => {
        const { bots } = this.state;
        this.setState({
    ...
    public onDecBotCount = (index: number) => {
        const { bots } = this.state;
        this.setState({
    ...
    public onSave = async () => {
        if (await this.check()) {
            const { dispatch } = this.props;
            const stage = StageConfigConverter.e2s({ custom: true, ...this.state });
    ...
    public getStage() {
        return StageConfigConverter.e2s({ custom: false, ...this.state });
    }

    public renderItemSwitchButtons() {
        return (
    ...
    public renderMapView() {
        ...
        return (
            <g className="map-view">
                <StagePreview disableImageCache={true} stage={this.getStage()} />
                <Grid t={t} />
    ...
    public renderConfigView() {
        ...
        return (
            <g className="config-view">
                ...
                <TextInput x={6.5 * B} y={B} maxLength={12} value={name} onChange={name => this.setState({ name })} />
                ...
                <g className="bots-config" transform={`translate(${6 * B}, ${4 * B})`}>
                    ...
                    <Text content={String(totalBotCount).padStart(2, "0")} x={4.5 * B} y={6 * B} fill="#ccc" />
                </g>
            </g>
        );
    }
