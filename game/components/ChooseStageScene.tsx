...
class ChooseStageScene extends React.PureComponent<{
    ...
    public onChoose = (stageName: string) => {
        const { dispatch, location } = this.props;
        dispatch(replace(`/choose/${stageName}${location.search}`));
    };

    public onChoosePrevStage = () => {
    ...
    public render() {
        ...
        return (
            <Screen background="#333">
                ...
                <StagePreview key={index + 1} stage={stages.get(index + 1)} x={12 * B} y={4.375 * B} scale={1 / 4} />
                <Text content={`stage ${stageName}`} x={6.5 * B} y={9.75 * B} />
                <g className="button-areas" transform={`translate(${2.5 * B}, ${12 * B})`}>
                    <TextButton content="prev" disabled={index === 0} x={0} y={0} onClick={this.onChoosePrevStage} />
                    <TextButton
                        content="next"
