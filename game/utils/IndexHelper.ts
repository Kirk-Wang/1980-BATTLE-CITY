...
export default class IndexHelper {
    ...
    public static resolveItemSize(type: ItemType) {
        if (type === "brick") {
    ...
    public static getT(type: ItemType, row: number, col: number) {
        const N = IndexHelper.resolveN(type);
        return row * N + col;
    }

    public static getRowCol(type: ItemType, t: number) {
        const N = IndexHelper.resolveN(type);
        return [Math.floor(t / N), t % N];
    }

    public static getPos(type: ItemType, t: number): Point {
    ...
    public static getRect(type: ItemType, t: number): Rect {
        const itemSize = IndexHelper.resolveItemSize(type);
        const [row, col] = IndexHelper.getRowCol(type, t);
        return {
            x: col * itemSize,
     ...
     */
    public static *iterRowCol(type: ItemType, rect: Rect, direction: Direction = "down") {
        const N = IndexHelper.resolveN(type);
        const itemSize = IndexHelper.resolveItemSize(type);
    ...
    public static *iter(type: ItemType, rect: Rect, direction: Direction = "down") {
        const N = IndexHelper.resolveN(type);
