...
export class BulletCollisionInfo extends DefaultMap<BulletId, Collision[]> {
    ...
    public static getCollisionRect(collision: Collision) {
    ...
    // 判断一颗子弹是否发生爆炸
    public static shouldExplode(collisions: Collision[]) {
    ...
    public bullets: IMap<BulletId, BulletRecord>;
    ...
    public getExplosionSoundName(bulletId: BulletId): SoundName {
    ...
    public getExplosionPos(bulletId: BulletId): Point {
        const collisions = this.get(bulletId);
        const bullet = this.bullets.get(bulletId);
