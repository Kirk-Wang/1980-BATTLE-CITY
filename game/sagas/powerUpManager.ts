...
import { BLOCK_SIZE, N_MAP, POWER_UP_NAMES, POWER_UP_SCORE, STAR_PICKED_BY_ARMOR_TANK_SCORE } from "../utils/constants";
...
function convertToSteels(map: MapRecord) {
    ...
    const nextSteels = steels.map((set, t) => (surroundingTTSet.has(t) && !eagleTTSet.has(t) ? true : set));
