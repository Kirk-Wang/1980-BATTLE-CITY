...
export function* directionCtrl(getPlayerName: any, getPlayerInput: (tank: TankRecord, delta: number) => Input) {
    while (true) {
        ...
        const player = yield select((s: State) => {
            // console.log(s);
            const playerName: string = getPlayerName();
            return s.players[playerName];
        });
        // console.log("player.activeTankId", player.activeTankId);
        const tank = yield select((s: State) => s.tanks.get(player.activeTankId));
